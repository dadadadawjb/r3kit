...
Then install `VIVE Hub` from <https://www.vive.com/vive-hub/download/>. Go to `Settings`, then `VIVE Ultimate Tracker`, `Pair new`.
Then `Set up`, where the most important is the `tracking map building`.
Currently, you must connect the trackers with computer by `VIVE Hub`.
